...
public class DinaAdminAuthTest {
  ...
  @Test
  @WithMockKeycloakUser(groupRole = {"GRP:SUPER_USER", "GRP:USER", "GRP:GUEST"})
  ...
  @Test
  @WithMockKeycloakUser(groupRole = {"GRP:SUPER_USER", "GRP:USER", "GRP:GUEST"})
  public void read_whenNotAdmin_AccessDenied() {
    assertThrows(AccessDeniedException.class, () -> testRepo.getAll(""));
  }

  @Test
  @WithMockKeycloakUser(adminRole = {"DINA_ADMIN"})
  public void read_whenAdmin_accessGranted() {
    testRepo.getAll("");
  }
  ...
  static class DinaAdminOnlyTestConfig {
    ...
    // we can't use the repo from ItemTestConfig since we need DinaAdminAuthorizationService
    @Bean
    @Primary
    public DinaRepositoryV2<ItemDto, Item> testRepo(
      Optional<AuditService> auditService,
      DinaAdminAuthorizationService authorizationService,
      ...
      return new DinaRepositoryV2<>(
        ...
        @Override
        public DinaRepositoryV2.PagedResource<JsonApiDto<ItemDto>> getAll(String queryString) {
          // try both
          authorizationService.authorize();
          authorizationService.authorizeRead("");
